# Relaxed Barrier Functions (RBFs): Soft Constraint and Penalty Function

- Reference literature for RBFs
    - <a name="tac15">[1]</a> [(IEEE Trans. Autom. Control 2017) Relaxed Logarithmic Barrier Function Based Model Predictive Control of Linear Systems](https://arxiv.org/pdf/1503.03314)

---

This note describes the relaxed barrier functions (RBFs) penalty with **self-collision** constraint handling as an example.

In this repo, RBF means a **relaxed logarithmic barrier penalty** <a href="#tac15">[1]</a> applied as a soft state inequality constraint.
...
So every configured pair contributes a penalty. 

The runtime hard stop is separate: it checks the minimum distance over all pairs and falls back to hold mode if that minimum is below `selfCollision.hardStopDistance`.
...
The figures in this section are generated by [`docs/assets/plot_rbf.py`](./assets/plot_rbf.py).
...
- [`ocs2_core/src/penalties/penalties/RelaxedBarrierPenalty.cpp`](https://github.com/wei-hsuan-cheng/ocs2_ros2/blob/humble_frame_relation/core/ocs2_core/src/penalties/penalties/RelaxedBarrierPenalty.cpp).
...
- [`ocs2_pinocchio/ocs2_self_collision/src/SelfCollision.cpp`](https://github.com/wei-hsuan-cheng/ocs2_ros2/blob/humble_frame_relation/robotics/ocs2_pinocchio/ocs2_self_collision/src/SelfCollision.cpp)
- [`ocs2_pinocchio/ocs2_self_collision/src/SelfCollisionCppAd.cpp`](https://github.com/wei-hsuan-cheng/ocs2_ros2/blob/humble_frame_relation/robotics/ocs2_pinocchio/ocs2_self_collision/src/SelfCollisionConstraintCppAd.cpp).

The dynamics MPC wrapper creates the self-collision constraint and attaches the RBF penalty here:

- [`dynamics_self_collision_constraint.cpp`](../src/common/constraint/dynamics_self_collision_constraint.cpp).
...
- [`inverse_dynamics_mpc_interface.cpp`](../src/inverse_dynamics_mpc/inverse_dynamics_mpc_interface.cpp)
- [`forward_dynamics_mpc_interface.cpp`](../src/forward_dynamics_mpc/forward_dynamics_mpc_interface.cpp).

The generated parameters are declared in:

- [`inverse_dynamics_mpc_controller_parameter.yaml`](../src/inverse_dynamics_mpc/inverse_dynamics_mpc_controller_parameter.yaml)
- [`forward_dynamics_mpc_controller_parameter.yaml`](../src/forward_dynamics_mpc/forward_dynamics_mpc_controller_parameter.yaml).