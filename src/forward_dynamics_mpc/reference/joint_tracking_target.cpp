  ...
  const TargetMsg& msg) const
{
  ...
  for (std::size_t sample = 0; sample < raw.stateTrajectory.size(); ++sample) {
    ...
    if (!raw.inputTrajectory.empty()) {
      const auto& raw_input = raw.inputTrajectory[sample];
      const Eigen::Index raw_input_size = raw_input.size();
      if (raw_input_size == 0) {
        // Empty per-sample inputs mean the target did not provide an explicit
        // torque reference. Keep the nonlinear-effects torque seed above.
      } else if (raw_input_size == static_cast<Eigen::Index>(n)) {
        for (std::size_t i = 0; i < n; ++i) {
          input(static_cast<Eigen::Index>(i)) =
            raw_input(static_cast<Eigen::Index>(reorder_indices[i]));
        }
      } else {
        throw std::runtime_error(
          "joint target input dimension must match the MPC joint dimension");
      }
    }
