  ...
  const std::vector<std::string>& modelNames)
{
  ...
  if (incomingNames.size() != n) {
    throw std::runtime_error(
      "joint_names size must match the MPC joint dimension when joint input references are provided");
  }
...
EeMotionTrackingTarget::EeMotionTrackingTarget(const InverseDynamicsMpcInterface& interface)
: interface_(interface)
{
}

bool EeMotionTrackingTarget::supports(const TargetMsg& msg) const noexcept
{
  return supportsCommandType(msg.command_type);
}

EeMotionTrackingTarget::TargetTrajectories EeMotionTrackingTarget::fromMessage(
  const TargetMsg& msg) const
{
  ...
  if (!msg.joint_acceleration_trajectory.empty() ||
      !msg.joint_torque_trajectory.empty()) {
  ...
  for (std::size_t sample = 0; sample < msg.state_trajectory.size(); ++sample) {
    ...
    if (!msg.joint_acceleration_trajectory.empty()) {
      const ocs2::vector_t raw_acceleration =
        vectorFromMessage(msg.joint_acceleration_trajectory[sample]);
      if (raw_acceleration.size() != static_cast<Eigen::Index>(n)) {
        throw std::runtime_error("joint_acceleration_trajectory sample dimension must match joint dimension");
      }
      for (std::size_t i = 0; i < n; ++i) {
        input(static_cast<Eigen::Index>(model.aOffset() + i)) =
          raw_acceleration(static_cast<Eigen::Index>(reorder_indices[i]));
      }
    }
