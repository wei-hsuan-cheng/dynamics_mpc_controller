        ...
        [&]() {
          ...
          if (reset_mpc_warm_start_requested_.exchange(false)) {
            ...
            RCLCPP_WARN_THROTTLE(
              ...
              *get_node()->get_clock(),
              status_log_period_ms_,
              "[InverseDynamicsMpcController] reset MPC warm start; next solve will use the current target and inverse-dynamics initializer.");
          }
...
bool InverseDynamicsMpcController::self_collision_hard_stop_is_triggered(
  const SystemObservation& observation,
  double& minimum_distance) const
{
  ...
  try {
    ...
    RCLCPP_WARN_THROTTLE(
      ...
      *get_node()->get_clock(),
      status_log_period_ms_,
      "[InverseDynamicsMpcController] self-collision hard-stop evaluation failed: %s",
      e.what());
    return true;
  }
